...
const formatOptions = [
    { value: 'text', label: 'نص عادي (TXT)' },
    { value: 'searchable_pdf', label: 'PDF قابل للبحث' },
    { value: 'word', label: 'ملف Word (DOCX)' },
];
...
function handleFileSelected(e) {
    ...
    if (selectedFile.type !== 'application/pdf' && !selectedFile.type.startsWith('image/')) {
        errorMessage.value = 'يرجى اختيار ملف PDF أو صورة';
        return;
    }
...
async function processOcr() {
    ...
    if (!file.value) {
        errorMessage.value = 'يرجى إضافة ملف أولاً.';
        return;
    }
    ...
    try {
        ...
        const result = await PdfProcessingService.processOcr(file.value, options);
        
        if (result.success) {
            if (outputFormat.value === 'text') {
                processedText.value = result.text;
            } else {
...
<template>
    <Head title="التعرف الضوئي على النصوص (OCR)" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">التعرف الضوئي على النصوص (OCR)</h2>
                <Link
        ...
        <div class="py-12 bg-gray-50">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <!-- Success State (Text Output) -->
                    <div v-if="processedText" class="mb-6">
                        <div class="mb-4 flex justify-between items-center">
                            <h3 class="text-lg font-medium text-gray-900">النص المستخرج:</h3>
                            <button 
                        ...
                        <div 
                            class="p-4 bg-gray-50 border rounded-md max-h-96 overflow-y-auto text-right"
                            dir="auto"
                        >
                            <pre class="whitespace-pre-wrap font-sans text-sm">{{ processedText }}</pre>
                        </div>
                    ...
                    <!-- Success State (File Output) -->
                    <div v-else-if="processedFileUrl" class="text-center">
                        <div class="mb-6">
                            ...
                            <h3 class="mt-4 text-xl font-medium text-gray-900">تم استخراج النص بنجاح!</h3>
                    ...
                    <div v-else>
                        <div class="text-center mb-8">
                            <h3 class="text-lg font-medium text-gray-900 mb-2">التعرف الضوئي على النصوص (OCR)</h3>
                            <p class="text-gray-600">قم بتحميل ملف PDF أو صورة لاستخراج النص منها.</p>
                        </div>
                        ...
                        >
                            ...
                            <p class="mt-1 text-sm text-gray-600">
                                اسحب وأفلت ملف PDF أو صورة هنا، أو
                                <label class="relative cursor-pointer text-blue-600 hover:text-blue-800">
                                    <span>انقر للاختيار</span>
                                    <input type="file" class="sr-only" accept="application/pdf,image/*" @change="handleFileSelected" />
                                </label>
                            </p>
                            <p class="mt-1 text-xs text-gray-500">يمكنك تحميل ملف PDF أو صورة (JPG, PNG, TIFF)</p>
                        </div>
                        ...
                        <div v-if="file" class="mt-6">
                            <div class="flex justify-between items-center bg-gray-50 p-4 rounded-md mb-6">
                                <div class="flex items-center">
                                    <svg v-if="file.type === 'application/pdf'" xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-red-500 ml-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                                    </svg>
                                    <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-blue-500 ml-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <h4 class="font-medium text-gray-900 mb-4">إعدادات التعرف الضوئي:</h4>
                            
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    <label for="language" class="block text-sm font-medium text-gray-700 mb-1">لغة النص</label>
                                    <select 
                                        id="language" 
                                        v-model="language" 
                                ...
                                <div>
                                    <label for="outputFormat" class="block text-sm font-medium text-gray-700 mb-1">صيغة الملف الناتج</label>
                                    <select 
                                        id="outputFormat" 
                ...
                <div class="mt-8 bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">كيفية استخدام خدمة التعرف الضوئي على النصوص</h3>
                    <div class="grid md:grid-cols-3 gap-6">
                        <div class="flex flex-col items-center text-center">
                            ...
                            <p class="text-gray-600 text-sm">قم بتحميل ملف PDF أو صورة تحتوي على نص.</p>
                        </div>
                        <div class="flex flex-col items-center text-center">
                            <div class="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mb-4">
                                <span class="text-blue-600 font-bold">2</span>
                            </div>
                            <h4 class="font-medium mb-2">اختر الإعدادات</h4>
                            <p class="text-gray-600 text-sm">حدد لغة النص وصيغة الملف الناتج.</p>
                        </div>
                        <div class="flex flex-col items-center text-center">
                            <div class="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mb-4">
                                <span class="text-blue-600 font-bold">3</span>
                            </div>
                            <h4 class="font-medium mb-2">استخرج النص</h4>
                            <p class="text-gray-600 text-sm">انقر على زر "استخراج النص" واحصل على النتيجة.</p>
                        </div>
                    </div>
                </div>
            </div>
