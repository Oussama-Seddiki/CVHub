<template>
    <AuthenticatedLayout title="تحويل PDF إلى JPG">
        <template #header>
                    ...
                    <div v-if="!processing && !downloadUrl">
                        ...
                        <div v-if="selectedFile" class="mt-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">جودة الصورة</label>
                                    <select v-model="imageQuality" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                        <option value="low">منخفضة (أصغر حجم)</option>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">دقة الصورة (DPI)</label>
                                    <select v-model="dpi" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                        <option value="72">72 DPI (للشاشة)</option>
                                        <option value="150">150 DPI</option>
                                        <option value="300">300 DPI (للطباعة)</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">خيارات إضافية</label>
                                    <div class="relative inline-block w-full">
                                        <label class="inline-flex items-center">
                                            <input type="checkbox" v-model="createZip" class="form-checkbox h-5 w-5 text-indigo-600 dark:text-indigo-400 rounded">
                                            <span class="mr-2 text-sm text-gray-700 dark:text-gray-300">إنشاء ملف ZIP (للملفات المتعددة)</span>
                                        </label>
                                    </div>
                                </div>
                            </div>
                            ...
                            >
                                ...
                                <span>تحويل إلى {{ outputFormat.toUpperCase() }}</span>
                            </button>
                        </div>
                    </div>
...
const processPdfToJpg = async () => {
    ...
    try {
        ...
        // استدعاء API
        const operationName = outputFormat.value === 'png' ? 'pdf-to-png' : 'pdf-to-jpg';
        const result = await PdfProcessingService.processImages(selectedFile.value, operationName, options);
        ...
        if (result && result.success && result.file) {
            ...
            error.value = result.message || 'حدث خطأ أثناء تحويل الملف.';
            processing.value = false;
        } else {
            throw new Error('لم يتم استلام بيانات الملف بشكل صحيح');
        }
    } catch (err) {
        console.error('Error processing PDF to JPG:', err);
        error.value = typeof err === 'string' ? err : (err.message || 'حدث خطأ أثناء تحويل الملف. يرجى المحاولة مرة أخرى.');
        if (processingInterval.value) {
            clearInterval(processingInterval.value);
            processingInterval.value = null;
        }
