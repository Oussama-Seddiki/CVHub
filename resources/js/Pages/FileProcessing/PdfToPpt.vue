<template>
    <AuthenticatedLayout title="تحويل PDF إلى PowerPoint">
        <template #header>
                    ...
                    <div v-if="!processing && !downloadUrl">
                        ...
                        <div v-if="selectedFile" class="mt-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">تنسيق العرض</label>
                                    <select v-model="outputFormat" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                        <option value="pptx">PPTX (PowerPoint الحديث)</option>
                                        <option value="ppt">PPT (PowerPoint القديم)</option>
                                    </select>
                                </div>

                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">نوع التحويل</label>
                                    <select v-model="conversionType" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                        <option value="per_page">شريحة لكل صفحة</option>
                                        <option value="text_extraction">استخراج النص والصور</option>
                                    </select>
                                </div>
                            ...
                            >
                                ...
                                <span>تحويل إلى PowerPoint</span>
                            </button>
                        </div>
                    </div>
...
const conversionType = ref('per_page');
...
const reset = () => {
    ...
    outputFormat.value = 'pptx';
    conversionType.value = 'per_page';
    pageOption.value = 'all';
    pageRange.value = '';
    enableOcr.value = false;
};

const processPdfToPpt = async () => {
    ...
    try {
        ...
        // استدعاء API
        const result = await PdfProcessingService.processDocuments(selectedFile.value, 'pdf-to-ppt', options);
        ...
        console.error('Error processing PDF to PowerPoint:', err);
