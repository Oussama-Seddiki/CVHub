...
async function processPdf() {
    ...
    try {
        ...
        const result = await PdfProcessingService.processPdf(file.value, 'protect', {
            password: password.value,
            permissions: {
                printing: allowPrinting.value,
                copying: allowCopying.value,
                modifying: allowModifying.value
            }
        });
    ...
    try {
        ...
        apiStatus.value = {
            ...
            message: status.ready
                ? 'خدمة معالجة ملفات PDF جاهزة للاستخدام'
                : 'خدمة معالجة ملفات PDF غير متاحة حالياً. يرجى التحقق من إعدادات API.'
        };
    } catch (error) {
        apiStatus.value = {
            ready: false,
