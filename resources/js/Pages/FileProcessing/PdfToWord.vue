<template>
    <AuthenticatedLayout title="تحويل PDF إلى Word">
        <template #header>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <select v-model="outputFormat" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                        <option value="docx">DOCX (Word الحديث)</option>
                                        <option value="doc">DOC (Word القديم)</option>
                                    </select>
                                </div>
...
const processPdfToWord = async () => {
    ...
    try {
        ...
        // استدعاء API
        const result = await PdfProcessingService.processDocuments(selectedFile.value, 'pdf-to-word', options);
        ...
        console.error('Error processing PDF to Word:', err);
